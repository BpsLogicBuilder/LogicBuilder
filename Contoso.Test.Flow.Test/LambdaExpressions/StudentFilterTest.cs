...
namespace Contoso.Test.Flow.Test.LambdaExpressions
{
    ...
    {
        static StudentFilterTest()
        {
            InitializeMapperConfiguration();
        }

        public StudentFilterTest(DatabaseFixture databaseFixture, ITestOutputHelper output)
        {
        ...
        [Theory]
        [InlineData("filter-student-against-value-source-member", "$it => ($it.ID == Contoso.Domain.Entities.StudentModel.ID)")]
        [InlineData("filter-student-against-value", "$it => ($it.ID == 1)")]
        public async Task SetFilter(string filterName, string filterString)
        {
            ...
            stopWatch.Stop();
            this.output.WriteLine("Get student filter = {0}", stopWatch.Elapsed.TotalMilliseconds);
