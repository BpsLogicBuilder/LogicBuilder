...
{
    public class ConstructorTreeFolderBuilderTest
    {
        public ConstructorTreeFolderBuilderTest()
		{
			Initialize();
		}
		...
		[Fact]
		public void CanCreateConstructorTreeFolder()
		{
			//arrange
			IConstructorTreeFolderBuilder constructorTreeFolderBuilder = serviceProvider.GetRequiredService<IConstructorTreeFolderBuilder>();

			//act
			var result = constructorTreeFolderBuilder.GetTreeFolder(GetDocument());

			//assert
			Assert.Equal("folderName", result.FolderNames[0].Name);
			Assert.Equal(2, result.FolderNames[0].FileNames.Count);
